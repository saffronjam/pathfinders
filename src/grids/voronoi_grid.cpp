...
void VoronoiGrid::OnRender(Scene &scene)
{
	...
	scene.Submit(_grid, sf::RenderStates::Default);
...
void VoronoiGrid::ClearNodeColor(int uid)
{
	if (uid == -1)
	{
		Log::CoreError("Cannot clear Voronoi node color: invalid uid");
		return;
	}
...
void VoronoiGrid::SetNodeColor(int uid, const sf::Color &color)
{
	if (uid == -1)
	{
		Log::CoreError("Cannot set Voronoi node color: invalid uid");
		return;
	}
...
void VoronoiGrid::ClearNodeEdgeColor(int fromUid, int toUid)
{
	...
	{
		Log::CoreError("Cannot clear Voronoi edge color: invalid uid");
		return;
	}
...
void VoronoiGrid::SetNodeEdgeColor(int fromUid, int toUid, const sf::Color &color)
{
	if (fromUid == -1 || toUid == -1)
	{
		Log::CoreError("Cannot set Voronoi edge color: invalid uid");
		return;
	}
