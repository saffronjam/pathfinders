...
void SquareGrid::OnRender(Scene &scene)
{
	...
	{
		scene.Submit(_lineVA, sf::RenderStates::Default);
	}

	if ( _drawFlags & TraverseGridDrawFlag_Objects )
	{
		scene.Submit(_filledSquaresVA, sf::RenderStates::Default);
		scene.Submit(_filledEdgesVA, sf::RenderStates::Default);
	}
}
...
void SquareGrid::ClearNodeColor(int uid)
{
	if (uid == -1)
	{
		Log::CoreError("Cannot clear square node color: invalid uid");
		return;
	}
...
void SquareGrid::SetNodeColor(int uid, const sf::Color &color)
{
	if (uid == -1)
	{
		Log::CoreError("Cannot set square node color: invalid uid");
		return;
	}
...
{
	if (fromUid == -1 || toUid == -1)
	{
		Log::CoreError("Cannot clear square edge color: invalid uid");
		return;
	}
...
void SquareGrid::SetNodeEdgeColor(int fromUid, int toUid, const sf::Color &color)
{
	if (fromUid == -1 || toUid == -1)
	{
		Log::CoreError("Cannot set square edge color: invalid uid");
		return;
	}
