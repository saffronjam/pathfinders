...
void Pathfinder::OnRenderAnticipation(Scene& scene)
{
		...
		PathfinderState::Finished)
	{
		...
		scene.Submit(_bodyVA, sf::RenderStates::Default);
...
void Pathfinder::OnRenderViaConnections(Scene& scene)
{
	...
	scene.Submit(_viaVA, sf::RenderStates::Default);
}
...
auto Pathfinder::StateString() const -> std::string
{
	...
	Log::CoreError("Invalid pathfinder state");
	return "";
}
...
auto Pathfinder::FinalCost() -> float
{
	...
	if (_finalPath.empty())
	{
		Log::CoreError("Cannot calculate final path cost before a path is found");
		return 0.0f;
	}
...
void Pathfinder::RenderFinishedBodyHelper(Scene& scene, sf::Color color, int limit)
{
	...
	scene.Submit(_bodyFinishedVA, sf::RenderStates::Default);
}
