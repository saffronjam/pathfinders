...
#include "grids/square_grid.h"
#include "grids/voronoi_grid.h"

#include "pathfinders/a_star.h"
#include "pathfinders/dijkstra.h"
#include "pathfinders/best_first_search.h"
...
void PathfinderManager::OnUpdate(Scene& scene)
{
	...
	if (!_working)
	{
		...
		{
			const auto mouseInViewportPanePosition = scene.Camera().ScreenToWorld(scene.ViewportPane().MousePosition(false));
...
void PathfinderManager::OnRender(Scene& scene)
{
	...
	if (!_working)
	{
		...
		if (_editState == PathfinderManagerEditState::Obstacles && _obstacleBrushEnabled)
		{
			...
			const auto position = scene.Camera().ScreenToWorld(scene.ViewportPane().MousePosition(false));
...
void PathfinderManager::OnGuiRender()
{
	...
	Gui::BeginPropertyGrid("Grid", -1.0f);
	...
	Gui::BeginPropertyGrid("MazeGeneration", -1.0f);
	...
	Gui::BeginPropertyGrid("Time", -1.0f);
	...
	Gui::BeginPropertyGrid("Edit", -1.0f);
