...
auto TraverseGrid::ClosestNeighborUID(int uid, const sf::Vector2f& position) const -> int
{
	...
	if (neighbors.empty())
	{
		Log::CoreError("Cannot query closest neighbor before neighbors have been generated");
		return -1;
	}
	...
	if (closestUID == -1)
	{
		Log::CoreError("Failed to find nearest neighbor node");
		return -1;
	}
	return closestUID;
}
...
auto TraverseGrid::NodeUidByPosition(const sf::Vector2f& position) const -> int
{
	...
	if (closestUID == -1)
	{
		Log::CoreError("Failed to find node by position");
		return -1;
	}
	return closestUID;
}
...
void TraverseGrid::OnRenderWeights(Scene& scene)
{
	...
	scene.Submit(_weightLinesVA, sf::RenderStates::Default);
}
